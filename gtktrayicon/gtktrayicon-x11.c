...
static void gtkti_tray_icon_constructed   (GObject     *object);
static void gtkti_tray_icon_dispose       (GObject     *object);

static void gtkti_tray_icon_get_property  (GObject     *object,
				 	 guint        prop_id,
					 GValue      *value,
					 GParamSpec  *pspec);

static void     gtkti_tray_icon_realize   (GtkWidget   *widget);
static void     gtkti_tray_icon_style_updated (GtkWidget   *widget);
static gboolean gtkti_tray_icon_delete    (GtkWidget   *widget,
					 GdkEventAny *event);
static gboolean gtkti_tray_icon_draw      (GtkWidget   *widget,
                                         cairo_t     *cr);

static void gtkti_tray_icon_clear_manager_window     (GtktiTrayIcon *icon);
static void gtkti_tray_icon_update_manager_window    (GtktiTrayIcon *icon);
static void gtkti_tray_icon_manager_window_destroyed (GtktiTrayIcon *icon);

static GdkFilterReturn gtkti_tray_icon_manager_filter (GdkXEvent *xevent,
						     GdkEvent  *event,
						     gpointer   user_data);


G_DEFINE_TYPE_WITH_PRIVATE (GtktiTrayIcon, gtkti_tray_icon, GTK_TYPE_PLUG)

static void
gtkti_tray_icon_class_init (GtktiTrayIconClass *class)
{
  GObjectClass *gobject_class = (GObjectClass *)class;
  GtkWidgetClass *widget_class = (GtkWidgetClass *)class;

  gobject_class->get_property = gtkti_tray_icon_get_property;
  gobject_class->constructed = gtkti_tray_icon_constructed;
  gobject_class->dispose = gtkti_tray_icon_dispose;

  widget_class->realize = gtkti_tray_icon_realize;
  widget_class->style_updated = gtkti_tray_icon_style_updated;
  widget_class->delete_event = gtkti_tray_icon_delete;
  widget_class->draw = gtkti_tray_icon_draw;
...
static void
gtkti_tray_icon_init (GtktiTrayIcon *icon)
{
  icon->priv = gtkti_tray_icon_get_instance_private (icon);
  icon->priv->stamp = 1;
...
static void
gtkti_tray_icon_constructed (GObject *object)
{
  /* Do setup that depends on the screen; screen has been set at this point */

  GtktiTrayIcon *icon = GTKTI_TRAY_ICON (object);
  ...
  gdk_window_add_filter (root_window,
			 gtkti_tray_icon_manager_filter, icon);

  gtkti_tray_icon_update_manager_window (icon);
}

static void
gtkti_tray_icon_clear_manager_window (GtktiTrayIcon *icon)
{
  ...
  if (icon->priv->manager_window != None)
    {
      ...
      gdk_window_remove_filter (gdkwin, gtkti_tray_icon_manager_filter, icon);
...
static void
gtkti_tray_icon_dispose (GObject *object)
{
  GtktiTrayIcon *icon = GTKTI_TRAY_ICON (object);
  GtkWidget *widget = GTK_WIDGET (object);
  GdkWindow *root_window = gdk_screen_get_root_window (gtk_widget_get_screen (widget));

  gtkti_tray_icon_clear_manager_window (icon);

  gdk_window_remove_filter (root_window, gtkti_tray_icon_manager_filter, icon);

  G_OBJECT_CLASS (gtkti_tray_icon_parent_class)->dispose (object);
}

static void
gtkti_tray_icon_get_property (GObject    *object,
			    guint       prop_id,
			    GValue     *value,
			    GParamSpec *pspec)
{
...
static gboolean
gtkti_tray_icon_draw (GtkWidget *widget, 
		    cairo_t   *cr)
{
  GtktiTrayIcon *icon = GTKTI_TRAY_ICON (widget);
  ...
  if (GTK_WIDGET_CLASS (gtkti_tray_icon_parent_class)->draw)
    retval = GTK_WIDGET_CLASS (gtkti_tray_icon_parent_class)->draw (widget, cr);
...
static void
gtkti_tray_icon_get_orientation_property (GtktiTrayIcon *icon)
{
...
static void
gtkti_tray_icon_get_visual_property (GtktiTrayIcon *icon)
{
  GdkScreen *screen = gtk_widget_get_screen (GTK_WIDGET (icon));
  GdkDisplay *display = gdk_screen_get_display (screen);
  Display *xdisplay = GDK_DISPLAY_XDISPLAY (display);
...
static void
gtkti_tray_icon_get_colors_property (GtktiTrayIcon *icon)
{
  GdkScreen *screen = gtk_widget_get_screen (GTK_WIDGET (icon));
  GdkDisplay *display = gdk_screen_get_display (screen);
  Display *xdisplay = GDK_DISPLAY_XDISPLAY (display);
...
static void
gtkti_tray_icon_get_padding_property (GtktiTrayIcon *icon)
{
  GdkScreen *screen = gtk_widget_get_screen (GTK_WIDGET (icon));
  GdkDisplay *display = gdk_screen_get_display (screen);
  Display *xdisplay = GDK_DISPLAY_XDISPLAY (display);
...
static void
gtkti_tray_icon_get_icon_size_property (GtktiTrayIcon *icon)
{
  GdkScreen *screen = gtk_widget_get_screen (GTK_WIDGET (icon));
  GdkDisplay *display = gdk_screen_get_display (screen);
  Display *xdisplay = GDK_DISPLAY_XDISPLAY (display);
...
gtkti_tray_icon_manager_filter (GdkXEvent *xevent,
			      GdkEvent  *event,
			      gpointer   user_data)
{
  GtktiTrayIcon *icon = user_data;
  ...
  if (xev->xany.type == ClientMessage &&
	  ...
	  xev->xproperty.atom == icon->priv->orientation_atom)
	{
	  ...
	  gtkti_tray_icon_get_orientation_property (icon);
	}
               ...
               xev->xproperty.atom == icon->priv->colors_atom)
        {
          ...
          gtkti_tray_icon_get_colors_property (icon);
        }
      else if (xev->xany.type == PropertyNotify &&
               xev->xproperty.atom == icon->priv->padding_atom)
        {
          gtkti_tray_icon_get_padding_property (icon);
        }
      else if (xev->xany.type == PropertyNotify &&
               xev->xproperty.atom == icon->priv->icon_size_atom)
        {
          gtkti_tray_icon_get_icon_size_property (icon);
        }
      else if (xev->xany.type == DestroyNotify)
	{
	  ...
	  gtkti_tray_icon_manager_window_destroyed (icon);
	}
      else
        {
          GTK_NOTE (PLUGSOCKET,
...
static void
gtkti_tray_icon_send_manager_message (GtktiTrayIcon *icon,
				    long         message,
				    Window       window,
				    long         data1,
				    long         data2,
...
static void
gtkti_tray_icon_update_manager_window (GtktiTrayIcon *icon)
{
  ...
  if (icon->priv->manager_window != None)
    {
      ...
      gdk_window_add_filter (gdkwin, gtkti_tray_icon_manager_filter, icon);

      gtkti_tray_icon_get_orientation_property (icon);
      gtkti_tray_icon_get_visual_property (icon);
      gtkti_tray_icon_get_colors_property (icon);
      gtkti_tray_icon_get_padding_property (icon);
      gtkti_tray_icon_get_icon_size_property (icon);
      ...
      if (gtk_widget_get_realized (GTK_WIDGET (icon)))
	{
	  if ((icon->priv->manager_visual == NULL &&
	      ...
	      (icon->priv->manager_visual == gtk_widget_get_visual (widget)))
	    {
	       ...
	       */
	      gtkti_tray_icon_send_dock_request (icon);
	    }
	  else
	    {
...
static void
gtkti_tray_icon_manager_window_destroyed (GtktiTrayIcon *icon)
{
...
static gboolean
gtkti_tray_icon_delete (GtkWidget   *widget,
		      GdkEventAny *event)
{
...
static void
gtkti_tray_icon_set_visual (GtktiTrayIcon *icon)
{
  GdkScreen *screen = gtk_widget_get_screen (GTK_WIDGET (icon));
...
static void
gtkti_tray_icon_realize (GtkWidget *widget)
{
  GtktiTrayIcon *icon = GTKTI_TRAY_ICON (widget);
  GdkWindow *window;

  /* Set our visual before realizing */
  gtkti_tray_icon_set_visual (icon);

  GTK_WIDGET_CLASS (gtkti_tray_icon_parent_class)->realize (widget);
  window = gtk_widget_get_window (widget);
  if (icon->priv->manager_visual_rgba)
    {
...
static void
gtkti_tray_icon_style_updated (GtkWidget   *widget)
{
...
guint
gtkti_tray_icon_send_message (GtktiTrayIcon *icon,
			     gint         timeout,
			     const gchar *message,
			     gint         len)
{
  ...
  g_return_val_if_fail (GTKTI_IS_TRAY_ICON (icon), 0);
  ...
  gtkti_tray_icon_send_manager_message (icon, SYSTEM_TRAY_BEGIN_MESSAGE,
...
void
gtkti_tray_icon_cancel_message (GtktiTrayIcon *icon,
			       guint        id)
{
  g_return_if_fail (GTKTI_IS_TRAY_ICON (icon));
  g_return_if_fail (id > 0);
  
  gtkti_tray_icon_send_manager_message (icon, SYSTEM_TRAY_CANCEL_MESSAGE,
				      (Window)gtk_plug_get_id (GTK_PLUG (icon)),
				      id, 0, 0);
}

GtktiTrayIcon *
gtkti_tray_icon_new_for_screen (GdkScreen  *screen, 
			       const gchar *name)
{
  g_return_val_if_fail (GDK_IS_SCREEN (screen), NULL);

  return g_object_new (GTKTI_TYPE_TRAY_ICON, 
		       "screen", screen, 
		       "title", name, 
		       NULL);
}
...
GtktiTrayIcon*
gtkti_tray_icon_new (const gchar *name)
{
  return g_object_new (GTKTI_TYPE_TRAY_ICON, 
		       "title", name, 
		       NULL);
...
GtkOrientation
gtkti_tray_icon_get_orientation (GtktiTrayIcon *icon)
{
  g_return_val_if_fail (GTKTI_IS_TRAY_ICON (icon), GTK_ORIENTATION_HORIZONTAL);

  return icon->priv->orientation;
}

gint
gtkti_tray_icon_get_padding (GtktiTrayIcon *icon)
{
  g_return_val_if_fail (GTKTI_IS_TRAY_ICON (icon), 0);
...
gint
gtkti_tray_icon_get_icon_size (GtktiTrayIcon *icon)
{
  g_return_val_if_fail (GTKTI_IS_TRAY_ICON (icon), 0);
